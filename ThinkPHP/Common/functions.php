...
function T($template = '', $layer = '')
{
    ...
    if (!isset($baseUrl)) {
         ...
         * 
         */
        $TO_LOOK_THEME = cookie('TO_LOOK_THEME', '', array('prefix' => 'ICN'));
        if ($TO_LOOK_THEME) {
...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    // 解析子域名
    if (isset($host)) {
        if( (defined('DOMAIN') && DOMAIN == $_SERVER['HTTP_HOST'] && !$host) || $host == ''){
            $domain = DOMAIN;
        }else{
            $domain = $host . (strpos($host, '.') ? '' : (defined('DOMAIN') && DOMAIN == $_SERVER['HTTP_HOST'] ? '.'.DOMAIN : strstr($_SERVER['HTTP_HOST'], '.')));
        }
    } elseif ($domain === true) {
