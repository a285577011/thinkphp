...
namespace Think\Upload\Driver\Fastdfs;
...
class FastdfsStorage {
	...
	static function sign($sk, $ak, $data){
		...
		return $ak . ':' . self::Fastdfs_Encode($sign);
	}

	static function signWithData($sk, $ak, $data){
		$data = self::Fastdfs_Encode($data);
	...
	public function upload($config, $file, $replace = false){
		...
		$fileName = self::Fastdfs_escapeQuotes($fileName);
		...
		switch($type){
			...
			case 'doc':
				...
				if(isset($param['mode']))
					$url .= '/'.(int)$param['mode'];
				if($param['cssurl'])
					$url .= '/'. self::Fastdfs_Encode($param['cssurl']);
				break;
	...
	public function getList($query = array(), $path = ''){
		...
		$url = "{$this->server}/list?".http_build_query($query);
	...
	public function info($key){
		$key = trim($key);
		$url = "{$this->server}/stat/" . self::Fastdfs_Encode("{$this->bucket}:{$key}");
		$accessToken = $this->accessToken($url);
	...
	public function downLink($key){
		$key = urlencode($key);
		$key = self::Fastdfs_escapeQuotes($key);
	...
	public function rename($file, $new_file){
		$key = trim($file);
		$url = "{$this->server}/move/" . self::Fastdfs_Encode("{$this->bucket}:{$key}") .'/'. self::Fastdfs_Encode("{$this->bucket}:{$new_file}");
		trace($url);
		$accessToken = $this->accessToken($url);
		$response = $this->request($url, 'POST', array('Authorization'=>"QBox $accessToken"));
		return $response;
	...
	public function del($file){
		$key = trim($file);
		$url = "{$this->server}/delete/" . self::Fastdfs_Encode("{$this->bucket}:{$key}");
		$accessToken = $this->accessToken($url);
		$response = $this->request($url, 'POST', array('Authorization'=>"QBox $accessToken"));
		return $response;
	}
	...
	public function delBatch($files){
		$url = $this->server . '/batch';
		$ops = array();
		foreach ($files as $file) {
			$ops[] = "/delete/". self::Fastdfs_Encode("{$this->bucket}:{$file}");
		}
	...
	static function Fastdfs_Encode($str) {// URLSafeBase64Encode
		$find = array('+', '/');
		$replace = array('-', '_');
		return str_replace($find, $replace, base64_encode($str));
	}

	static function Fastdfs_escapeQuotes($str){
		$find = array("\\", "\"");
		$replace = array("\\\\", "\\\"");
