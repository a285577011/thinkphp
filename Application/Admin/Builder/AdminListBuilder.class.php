...
//            $this->setSearchPostUrl('');
    ...
    public function keyImages($name, $title)
    {
    	return $this->key($name, $title, 'images');
    } 
    ...
    public function display($solist = '')
    {
        ...
        $this->convertKey('images', 'html', function ($value, $key, $item) {
        		if (is_array($value)) {//value是图片id
        			foreach ($value as $v){
        			$value = htmlspecialchars($v);
        			//$sc_src = get_cover($v, 'path');
        			$src = getThumbImageById($v, 80, 80);
        			$sc_src = $src;
        			//echo $sc_src;die;
        			$html .= "<div class='popup-gallery'><a title=\"" . L('_VIEW_BIGGER_') . "\" href=\"$sc_src\"><img src=\"$sc_src\"/ style=\"width:80px;height:80px\"></a></div>";
        			 }
