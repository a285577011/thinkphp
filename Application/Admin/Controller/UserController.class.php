...
class UserController extends AdminController
{
    ...
    public function index()
    {
        ...
        $pos_province = I('pos_province', 0, 'intval');
        ...
        $pos_province && $map['pos_province'] = $pos_province;
        
        if ($aUserStatus) {//筛选状态
            $map['status'] = $aUserStatus == 2 ? 0 : $aUserStatus;
        }
       
        $list = $this->lists('Member', $map, 'uid ASC');
        int_to_string($list);
        foreach($list as $key=>$v){
            $list[$key]['ext']=query_user(array('username','mobile','avatar','qrcode','catid','tags'),$v['uid']);

        }
        $provinces = M('District')->where(array('level'=>1))->getField('id,name');

        $statusOptions = array(
                0 => array('id' => 0, 'value' => L('_ALL_')),
                1 => array('id' => 1, 'value' => L('_ENABLE_')),
                3 => array('id' => 2, 'value' => L('_DISABLE_')),
        );
    ...
    public function expandinfo_details($uid = 0)
    {
        if (IS_POST) {
            ...
            /* 微信名称 */
            if($data['nickname']){
                $userinfo = query_user(array('nickname'), $data['id']);
            ...
            /* 微信号 */
            if($data['weixin']){
                $userinfo = query_user(array('weixin'), $data['id']);
                if ($data['weixin'] != $userinfo['weixin']) {
                    $base['weixin'] = $data['weixin'];
                }
            }
            /* 微信号end */
            ...
            if($data['category']){
                ...
                if (($data['type'] != $userinfo['type']) || ($data['category'] != $userinfo['category'])) {
                    // 主关键词
                    $userCategoryModel = D('Ucenter/UserCategory');
                    $cat['type'] = $data['type'];
                    $cat['title'] = $data['category'];
                    
                    $catid = $userCategoryModel->addCategory($cat);
                    if ($catid && $data['catid'] != $userinfo['catid']) {
                        $base['catid'] = $catid;
                    }
                }
            }
            ...
            // 修改基本信息
            if($base){
                D('Member')->where(array('uid'=>$uid))->save($base);
                clean_query_user_cache($uid, array_keys($base));
            }
            
            /* 副标签 */
            if($data['tags']){
                $userinfo = query_user(array('tags'), $uid);
                if($userinfo['tags']){
                    foreach ($userinfo['tags'] as $k => $v){
                        $tagstr .= $v['title'].',';
                    }
                    $tagstr = trim($tagstr, ',');
                }
                ...
                if ($data['tags'] != $tagstr) {
                    $user_tag = str_replace("，", ',', $data['tags']);
                    $tags = array();
                    $ids = array();
                    ...
                    D('Ucenter/UserTagLink')->editData(implode(',', $ids), $uid);
            ...
            /* 头像 */
            if($data['avatar'] && is_numeric($data['avatar'])){
                D('Ucenter/Avatar')->saveAvatar(array('uid' => $uid, 'path' => $data['avatar']));
                clean_query_user_cache($uid, 'avatar');
            }
            /* 头像end */
            
            /* 二维码 */
            if($data['qrcode'] && is_numeric($data['qrcode'])){
                $map = getUserConfigMap('qrcode', '', $uid);
                D('Ucenter/UserConfig')->saveValue($map, $data['qrcode']);
                clean_query_user_cache($uid, 'qrcode');
            }
            /* 二维码end */
            ...
            $userinfo = query_user(array('username', 'catid', 'tags', 'weixin', 'avatar', 'qrcode'), $uid);
            $member['username'] = $userinfo['username'];
            $member['category'] = $userinfo['category'];
            $member['weixin'] = $userinfo['weixin'];
            $member['type'] = $userinfo['type'];
            $member['avatar'] = $userinfo['avatar'];
            $member['qrcode'] = $userinfo['qrcode'];
            if($userinfo['tags']){
                foreach ($userinfo['tags'] as $tk => $tv){
                    $tagstr .= $tv['title'].',';
                }
                $tagstr = trim($tagstr, ',');
            }
            ...
            // 基本信息
            $base_key = array('id', 'username', 'nickname', 'type', 'category', 'tags', 'avatar', 'qrcode');
            $builder->title(L('_USER_EXPAND_INFO_DETAIL_'));
            $builder->meta_title = L('_USER_EXPAND_INFO_DETAIL_');
            $builder->keyId()
                ->keyReadOnly('username', '账号')
                ->keyReadOnly('nickname', '微信名称')
                ->keyText('weixin', '微信号')
                ->keyRadio('type', '微商类型', '', array(1=>'商品', 2=>'非商品'))
                ->keyText('category', '主标签')
                ->keyText('tags', '副标签')
                ->keyAvatar('avatar', '头像')
                ->keyAvatar('qrcode', '二维码');
            ...
            $builder->group('扩展资料', implode(',', $field_key));
    ...
    public function profile($page = 1, $r = 20)
    {
        ...
        $builder->keyId()->keyText('profile_name', L('_GROUP_NAME_'))->keyText('sort', L('_SORT_'))->keyTime("create_time", L('_CREATE_TIME_'))->keyBool('visiable', L('_PUBLIC_IF_'));
    ...
    public function field($id, $page = 1, $r = 20)
    {
        ...
        $builder->keyTime("create_time", L('_CREATE_TIME_'))->keyStatus()->keyDoAction('User/editFieldSetting?profile_group_id=' . $id . '&id=###', L('_EDIT_'));
     ...
     * 系统头像方法
     */
    public function sysAvatar(){
        if(IS_POST){
            $data=array();
            $data['path']=intval(I('post.path'));
            $data['status']=1;
            $data['create_time']=time();
            if(D('Ucenter/SystemAvatar')->saveAvatar($data)){
                $this->ajaxReturn(array('status'=>1,'info'=>'保存成功!'));
            }
            $this->ajaxReturn(array('status'=>0,'info'=>'保存失败!'));
        }
        else{
            $data=D('Ucenter/SystemAvatar')->getList();            
            $this->assign('_list',$data);
            $this->display();
        }
    }
    public function changeSysAvatarStatus(){
        $id=I('post.id');
        $status=intval(I('get.status'));
        $msg=D('Ucenter/SystemAvatar')->changeStatus($id,$status);
