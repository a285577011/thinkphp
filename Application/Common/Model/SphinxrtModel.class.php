 ...
 * //查询
 * $prodList =
 * $sphinx->where($condition)->order($orderCondition)->group('prod_uid')->search();
 * //插入数据
...
class SphinxRtModel extends Model
{
 ...
 // 调试状态
     ...
     * @param
     *            param
     * @example select * from rt where match('keyword') group by gid WITHIN
     *          GROUP ORDER BY @weight DESC
     *          order by gid desc limit 0,1 option
     *          ranker=bm25,max_matches=3,field_weights=(title=10,content=3);
     * @return array +----------------------------------------------------------
     */
    public function search ()
    {
        ...
        $this->queryStr = sprintf("SELECT %s FROM %s %s %s %s %s %s", $field, 
                $this->_sql['rt'], $where, $groupSql, $orderSql, $limitSql, 
                $optionSql);
    ...
    public function insert ($data, $lastId = 0)
    {
        if (! emptyempty($data)) {
            ...
            $this->queryStr = "insert into " . $this->_sql['rt'] . "(id" .
                     $fields . ") values ($lastId {$values})";
            return $this->query();
        }
        $this->error = '插入数据不能为空';
        return false;
    ...
    public function insertAll ($datas, $asStr = true)
    {
        if (! emptyempty($datas)) {
            ...
            if ($asStr) {
                $values = ltrim($values, ',');
                $this->queryStr = sprintf(
                        "insert into {$this->_sql['rt']}(%s) values %s", $fields, 
                        $values);
                return $this->query();
            }
        } else {
    ...
    public function update ($data, $id, $insert = true)
    {
        if (! emptyempty($data) || $id > 0) {
            ...
            // 若该条数据不存在，直接插入
            $this->queryStr = "replace into " . $this->_sql['rt'] . "(id" .
    ...
    public function delete ($id)
    {
        if (is_array($id) && count($id) >= 1) {
            $rs = true;
            foreach ($id as $v) {
                $this->queryStr = sprintf("delete from %s where id=%d", 
                        $this->_sql['rt'], $v);
                $rs &= $this->query();
            }
        } else {
     ...
     * +----------------------------------------------------------
     * 
     * @todo 获取索引的字段值，前提条件是索引服务器中必须至少一个值，暂时没有api显示可以直接像mysql 的语句 desc table
     *       来获取索引的字段;
     * @access public
     * @param param
     * @return void
    ...
    public function triggerDebug ($debugMode = false)
    {
        if ($debugMode) {
            ...
            $errorStr .= 'line:' . $debugInfo[0]['line'];
            $errorStr .= 'sql:' . $debugInfo[0]['object']->queryStr;
            $errorStr .= 'error:' . $debugInfo[0]['object']->error . '';
