...
use Ucenter\Model\UserCategoryModel;
...
class MemberModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('weixin', 'require', '微信号必填', 0, '', self::MODEL_INSERT),
        array('weixin', '/^[a-zA-Z\d_]{5,}$/', '微信号不正确', self::MODEL_BOTH, 'regex'),
        /* 验证昵称 */
        array('nickname', 'checkNicknameLength', '微信名称长度不正确', self::EXISTS_VALIDATE, 'callback'), //昵称长度不合法
        array('nickname', 'checkDenyNickname', '微信名称禁止注册', self::EXISTS_VALIDATE, 'callback'), //昵称禁止注册
        array('nickname', 'checkNickname', '微信名名称格式不正确', self::EXISTS_VALIDATE, 'callback'),
        //array('nickname', '', -30, self::EXISTS_VALIDATE, 'unique'), //昵称被占用
        array('sex', 'require', '性别必填', 0, '', self::MODEL_INSERT),
        array('sex', array(1, 2), '性别不正确', self::EXISTS_VALIDATE, 'in'),
    );
    protected $insertField = 'nickname,sex,weixin,catid'; //新增数据时允许操作的字段
    protected $updateField = 'nickname,sex,last_login_ip,login,update_time,last_login_role,show_role,status,tox_money,score,pos_province,pos_city,pos_district,pos_community'; //编辑数据时允许操作的字段

    /* 缓存key设置 */
    protected $_skey_main = 'member_{id}'; // 主键
    protected $_skey_list_newest = 'member_list_newest_{number}'; // 最新微商
    protected $_skey_list_hot = 'member_list_hot_{number}'; // 微商红人
     ...
     * 获取首页商品微商关键词推荐缓存key
     */
    public function getCacheKeyListNewest($number) {
        return str_replace('{number}', $number, $this->_skey_list_newest);
    }

    /**
     * 获取首页非商品微商关键词推荐缓存key
     */
    public function getCacheKeyListHot($number) {
        return str_replace('{number}', $number, $this->_skey_list_hot);
    }
     ...
     * 
     * @param array $data            
     * @return boolean|Ambigous <mixed, boolean, unknown, string>|string
     */
    public function registerMember($data) {
        ...
        if ($user) {
            ...
            // $this->initFollow($uid); //默认加关注，先注释掉，需要再开启 dpj 2016-03-23
            return $uid;
        } else {
    ...
    private function autoLogin($user, $remember = false, $role_id = 0) {
        ...
        if (!$this->getCookieUid() && $remember) {
            $expire = 3600 * 24 * 7;
            cookie('login_token', $this->jiami($this->change() . ".{$user['uid']}.{$token}"), $expire);
        }
    }
    ...
    public function addSyncData($uid, $info) {
        ...
        $rule = array(
                ...
                array('nickname', '', -30, self::EXISTS_VALIDATE, 'unique')
        );
        $data = $this->validate($rule)->create($data);
        $data['uid'] = $uid;
        $res = $this->add($data);
        return $res;
    }
     ...
     */
    public function getDegree($uid) {
        if ($uid) {
            //$score = $this->getScore();
        }
        // TODO 等级配置表
        //$degree = D('UserDegree')->getDegree($score);
        $degree = rand(1, 10);
        return $degree;
    }
    ...
    public function getMemberList($param) {
        ...
        //$filter['tags'] = array(2,7);
        return $sphinxModel->search('member', $param['keyword'], $param);
    }
    ...
    public function getInfoById($id) {
        ...
        $data = $this->where(array('id' => $id))->find(array('cache' => array('key' => $skey)));
        return $data;
    }

    /**
     * 最新微商
     * @param number $number 数量,默认8个
     */
    public function newestList($number = 8) {
        $skey = $this->getCacheKeyListNewest($number);
        $data = S($skey);
        if (!$data) {
            ...
            //$param['field'] = 'uid,nickname,sex,reg_time';
            $res = $this->getMemberList($param);

            foreach ($res['list'] as $k => $v) {
                $data[] = query_user(array('avatar', 'catid', 'nickname', 'sex', 'reg_time', 'username'), $v['id']);
            }
     ...
     * @param number $number
     */
    public function hotList($number = 16) {
        $skey = $this->getCacheKeyListHot($number);
        $data = S($skey);
        if (!$data) {
            // TODO 暂时读库，待优化搜素引擎算法，从搜索引擎读取
            $param['where']['status'] = 1;
            $param['limit'] = $number;
            $param['order'] = '@random'; // 随机，暂时没有红人
            //$param['field'] = 'uid,nickname,sex';
            $res = $this->getMemberList($param);

            foreach ($res['list'] as $k => $v) {
                $data[] = query_user(array('avatar', 'nickname', 'sex', 'avatar64', 'key', 'weixin', 'catid', 'ipai', 'huoyuan', 'level', 'username'), $v['id']);
            }
            S($skey, $data, 600); // 缓存十分钟
        }
        return $data;
     ...
     * @param number $number
     */
    public function similarList($catid, $number = 16) {
        $skey = $this->getCacheKeyListHot(md5($catid . '_' . $number));
        $data = S($skey);
        if (!$data) {
            $param['where']['status'] = 1;
            $param['where']['catid'] = $catid;
            $param['limit'] = $number;
            $param['order'] = '@random'; // 随机，暂时没有红人
            $rows = $this->getList($param);
            foreach ($rows as  $v) {
                $data[] = query_user(array('avatar', 'nickname', 'sex', 'avatar64', 'key', 'weixin', 'catid', 'ipai', 'huoyuan', 'level', 'username'), $v);
            }
            S($skey, $data, 600); // 缓存十分钟           
        }
        return $data;
     ...
     * @param type $num 数量
     */
    public function upScore2($uid, $type, $num) {
        if ($type == 0) {
            return $this->where(array("uid" => $uid))->setInc('score2', $num);
        }
        if ($type == 1) {
            return $this->where(array("uid" => $uid))->setDec('score2', $num);
        }
        return FALSE;
    }
