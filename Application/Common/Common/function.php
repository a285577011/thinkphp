...
require_once(APP_PATH . '/Common/Common/follow.php');
...
/**
 * 导入函数文件
 * 如：GF('Common/function','Common') 导入App/Common/Common/function.php 
 * @staticvar array $_model_common
 * @param type $name 资源目录
 * @param type $layer 模块
 * @param type $ext 扩展名
 * @return array
 * @author zhangby
 */
function GF($name = '', $layer = '', $ext = '.php') {
    if (empty($name))
        E('资源地址为空');
    static $_model_common = array();
    $layer = $layer ? $layer : 'Common';
    if (isset($_model_common[$name . $layer]))
        return $_model_common[$name . $layer];
    $file = APP_PATH . $layer . '/' . $name . $ext;
    if (!file_exists($file)) {
        E('资源' . $file . '不存在。');
    }
    $_model_common[$name . $layer] = require_cache($file);
    return $_model_common[$name . $layer];
}
...
function convert_url_query($query) {
    if (!empty($query)) {
        ...
        foreach ($queryParts as $param) {
            $item = explode('=', $param);
            foreach ($item as $k=>$v){
            $params[$item[0]] .= $item[$k+1].'=';
            }
            $params[$item[0]]=rtrim($params[$item[0]],'=');
        }
        return $params;
    }
    return '';
...
function get_user_ext_info($uid) {
    //读取扩展字段
    $ext_field = D('FieldSetting')->getFieldList(array('status' => 1));
    $user_ext = array();
    foreach ($ext_field as &$v) {
        $user_ext[$v['id']] = array('field_name' => $v['field_name']);
        $user_ext[$v['id']]['visiable'] = 0;
        $user_ext[$v['id']]['data'] = '';
    }


    //赋值扩展字段名
    $ext = D('Field')->getFieldByUid($uid);
    foreach ($ext as &$v) {
        $v['icon'] = get_field_icon('field_' . $v['field_id']);
        if (isset($user_ext[$v['field_id']])) {
            $user_ext[$v['field_id']]['data'] = $v['field_data'];
            $user_ext[$v['field_id']]['visiable'] = $v['visiable'];
            $user_ext[$v['field_id']]['value'] = $v;
        }
    }
...
//取得用户字段ICON
function get_user_field_icon($user) {
    $ext = array();
    foreach ($user as $k => $v) {
        $ext[$k] = get_field_icon($k);
    }
    return $ext;
}
 ...
 * @param type $data
 */
function get_field_icon($field_name) {
    $icon = D('UserFieldIcon')->getRowList();

    $icons = array();
    foreach ($icon as $v) {
        $icons[$v['field_name']] = $v['icon'];
    }

    return isset($icons[$field_name]) ? $icons[$field_name] : '';
}

/**
 * 取得用户字段是否公开显示
 */
function get_user_field_show($uid) {
    $result = D('UserFieldSetting')->getRow($uid);
    if (empty($result))
        return null;
...
//获取用户信息
function get_user_info($uid = null, $ext = FALSE) {
    $user_info = query_user(array('weiquanRemark','score2', 'ipai', 'ishot', 'qrcode', 'balance', 'catid', 'avatar', 'avatar64', 'avatar128', 'weixin', 'username', 'nickname', 'email', 'uid', 'space_url', 'score', 'level', 'title', 'fans', 'pos_province', 'pos_city', 'pos_district', 'pos_community', 'following', 'weiquancount', 'rank_link', 'signature', 'sex', 'tags', 'expand_info'), $uid);
    //获取用户封面id
    ...
    $user_info['step'] = D('Ucenter/UserStep')->getStepPercentage($uid);
    $user_info['cat_data'] = D('Ucenter/UserCategory')->getById($user_info['catid']);

    if ($user_info['pos_province'] != 0) {
        $user_info['pos_province'] = D('District')->getNameById($user_info['pos_province']);
        $user_info['pos_city'] = D('District')->getNameById($user_info['pos_city']);
        $user_info['pos_district'] = D('District')->getNameById($user_info['pos_district']);
        $user_info['pos_community'] = D('District')->getNameById($user_info['pos_community']);
    }
    if ($ext) {
        $user_info['ext'] = get_user_ext_info($uid);
    }
    return $user_info;
}
...
function generate_promotion_code($no_of_codes, $exclude_codes_array = '', $code_length = 4) {
    ...
    for ($j = 0; $j < $no_of_codes; $j ++) {
        ...
        if (!in_array($code, $promotion_codes)) {
            if (is_array($exclude_codes_array)) { // 排除已经使用的优惠码
 ...
 */
function weixin_fans() {
    $counter = S('weixin_fans_counter');
    if (!$counter) {
        $counter = M('weixinFansCounter')->getField('num');
        S('weixin_fans_counter', $counter, 60); // 缓存一分钟
    }
    return $counter;
}
