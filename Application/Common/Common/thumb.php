...
function get_cover($cover_id, $field = null)
{
    ...
    return D('Common/File')->getFilePath($cover_id);
}
 ...
 * @param int $type 图片的类型，qiniu 七牛，local 本地, sae SAE, fastdfs
 * @param bool $replace 是否强制替换
 * @return string
 * 
 */
function getThumbImageById($file_id, $width = 100, $height = 'auto', $type = 0, $replace = false)
{
    !$type && $type = C('DOWNLOAD_UPLOAD_DRIVER');
    switch (strtolower($type)){
        case 'fastdfs':
            $picture = D('Common/File')->getFilePath($file_id);
            if (empty($picture)) {
                return get_pic_src('Public/images/nopic.png');
            }
            return get_pic_src($picture);
            break;
        case 'qiniu':
            break;
        default:
            $picture = S('picture_' . $file_id);
            if (empty($picture)) {
                $picture = M('Picture')->where(array('status' => 1))->getById($file_id);
                S('picture_' . $file_id, $picture);
            }
            if (empty($picture)) {
                return get_pic_src('Public/images/nopic.png');
            }
...
function get_pic_src($path)
{
    ...
    if ($not_http_remote && $not_https_remote) {
        //本地url
        return 'http://'.DOMAIN.str_replace('//', '/', '/'.str_replace('//', '/', getRootUrl() . $path)); //防止双斜杠的出现
    } else {
        //远端url
        return $path;
    }
