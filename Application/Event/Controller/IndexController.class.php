...
class IndexController extends Controller
{
    ...
    public function doPost($id = 0, $cover_id = 0, $title = '', $explain = '', $sTime = '', $eTime = '', $address = '', $limitCount = 0, $deadline = '', $type_id = 0)
    {
        ...
        if ($id) {
            ...
            if ($rs) {
                ...
                $this->success(L('_SUCCESS_DELETE_').L('_EXCLAMATION_'). $tip, U('index'));
            } else {
                $this->error(L('_ERROR_OPERATION_FAIL_').L('_EXCLAMATION_'));
            }
    ...
    public function doSign($event_id, $name, $phone)
    {
        ...
        if (!$check) {
            ...
            if ($res) {
                D('Message')->sendMessageWithoutCheckSelf($event_content['uid'], L('_TOAST_SIGN_1_'), query_user('nickname', is_login()) . L('_TOAST_SIGN_2_') . $event_content['title'] . L('_TOAST_SIGN_3_'), 'Event/Index/member', array('id' => $event_id));
    ...
    public function shenhe($uid, $event_id, $tip)
    {
        ...
        if ($tip) {
            ...
            D('Message')->sendMessageWithoutCheckSelf($uid, L('_MESSAGE_AUDIT_APPLY_1_'), query_user('nickname', is_login()) . L('_MESSAGE_AUDIT_APPLY_2_') . $event_content['title'] . L('_MESSAGE_AUDIT_APPLY_3_'), 'Event/Index/detail', array('id' => $event_id));
        } else {
            D('Event')->where(array('id' => $event_id))->setDec('attentionCount');
            D('Message')->sendMessageWithoutCheckSelf($uid, L('_MESSAGE_AUDIT_CANCEL_1_'), query_user('nickname', is_login()) . L('_MESSAGE_AUDIT_CANCEL_2_') . $event_content['title'] . L('_MESSAGE_AUDIT_CANCEL_3_'), 'Event/Index/member', array('id' => $event_id));
        }
        if ($res) {
    ...
    public function unSign($event_id)
    {
        ...
        if ($res) {
            ...
            D('Message')->sendMessageWithoutCheckSelf($event_content['uid'], L('_TOAST_CANCEL_1_'), query_user('nickname', is_login()) . L('_TOAST_CANCEL_2_') . $event_content['title'] . L('_TOAST_CANCEL_3_'), 'Event/Index/detail', array('id' => $event_id));
