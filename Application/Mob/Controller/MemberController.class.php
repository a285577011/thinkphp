...
//dump($aVerify);
 ...
 /*           if (!$aUsername) {
            ...
            if ($aEmail) {
                ...
                $content = C('USER_RESPASS') . "<br/>" . $url . "<br/>" . modC('WEB_SITE_NAME', 'I.CN-爱微商', 'Config') . "系统自动发送--请勿直接回复<br/>" . date('Y-m-d H:i:s', TIME()) . "</p>";
                send_mail($email, modC('WEB_SITE_NAME', 'I.CN-爱微商', 'Config') . "密码找回", $content);
...
/*                $user = UCenterMember()->where(array('username' => $username, 'status' => 1))->find();
    ...
    public function doSendVerify($account, $verify, $type)
    {
        switch ($type) {
            ...
            case 'email':
                ...
                $content = str_replace('{$verify}', $verify, $content);
                $content = str_replace('{$account}', $account, $content);
                $res = send_mail($account, modC('WEB_SITE_NAME', 'I.CN-爱微商', 'Config') . '邮箱验证', $content);
                return $res;
                break;
        }
    ...
    private function sendActivateEmail($account, $verify, $uid)
    {
        ...
        $content = str_replace('{$title}', modC('WEB_SITE_NAME', 'I.CN-爱微商', 'Config'), $content);
        $res = send_mail($account, modC('WEB_SITE_NAME', 'I.CN-爱微商', 'Config') . '激活信', $content);
