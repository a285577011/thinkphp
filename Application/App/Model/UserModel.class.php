...
class UserModel extends Model
{
	...
	public $type = array(1 => '商品微商', 2 => '非商品微商');
    ...
    protected $_validate = array(
        /* 验证用户名 */
        array('account', 'checkUsernameLength', '用户名长度不正确', self::EXISTS_VALIDATE,'callback'), //用户名长度不合法
        array('account', 'checkDenyMember', '用户名禁止注册', self::EXISTS_VALIDATE, 'callback'), //用户名禁止注册
        array('account', 'checkUsername', '用户名不合法', self::EXISTS_VALIDATE, 'callback'),
        array('account', '', '用户名已存在', self::EXISTS_VALIDATE, 'unique'), //用户名被占用
        
    	array('weixin','require','微信号必须', self::MUST_VALIDATE,'', self::MODEL_INSERT),
    	array('nickname','require','微信名必须', self::MUST_VALIDATE,'', self::MODEL_INSERT),

        /* 验证密码 */
    	array('password','require','请输入密码', self::MUST_VALIDATE,'', self::MODEL_INSERT),
        array('password', '6,30', '密码长度不正确', self::EXISTS_VALIDATE, 'length'), //密码长度不合法
        ...
        /* 验证手机号码 */
    	array('mobile','require','手机号必须', self::MUST_VALIDATE,'', self::MODEL_INSERT),
        array('mobile', '/^(1[3|4|5|8])[0-9]{9}$/', '手机格式不正确', self::EXISTS_VALIDATE), //手机格式不正确 TODO:
        array('mobile', 'checkDenyMobile', '手机禁止注册', self::EXISTS_VALIDATE, 'callback'), //手机禁止注册
        array('mobile', '', '手机号已被注册', self::EXISTS_VALIDATE, 'unique'), //手机号被占用
    );
    ...
    {
		$result = D ( 'Common/Member' )->registerMember ( $data ['account'] );
		if ($result > 0) {
			$data ['id'] = $result;
			$uid = $this->add ( $data );
			// TODO 事物处理
    ...
    public function getInfoById($uid)
    {
        ...
        if($user){
	        $user['last_login_time'] = time_format($user['last_login_time']);
	        $user['reg_time'] = time_format($user['reg_time']);
    ...
    public function lists($map){
    	// TODO 搜索接口
    	$data = M('User')->where($map)->select(); //->field('id,account,weixin,nickname,sex')
    	foreach ($data as $k => &$v){
    		unset($v['password'], $v['update_time'], $v['status'], $v['reg_from'], $v['catid']);
    		$v['last_login_time'] = time_format($v['last_login_time']);
    		$v['reg_time'] = time_format($v['reg_time']);
    		$v['last_login_ip'] = long2ip($v['last_login_ip']);
    		$v['reg_ip'] = long2ip($v['reg_ip']);
