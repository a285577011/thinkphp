...
class UploadAvatarWidget extends Controller
{
    ...
    public $avatar_size = array(array(120,120), array(300,300));
    ...
    {
        $avatar = D('Ucenter/Avatar')->getByUid($uid);
        if ($avatar) {
            ...
            if($avatar['driver'] == 'local'){
                ...
                if (class_exists($name)) {
                    ...
                    if (method_exists($class, 'thumb')) {
                        if(!in_array(array($size,$size), $this->avatar_size)){
                            // TODO 图片服务器自动缩略
                            //$size = 300;  // 不在允许头像规格内，默认300
                        	$size=0;
                        }
