...
1600:Lv7 ' . L('_CHAIRMAN__') . '
    ...
    public function avatar() {
        $info=query_user(array('avatar','qrcode','catid'),is_login());
        $sysAvatar=D('Ucenter/SystemAvatar')->getList(4);
        $this->assign('avatar',$info['avatar']);
        $this->assign('qrcode',$info['qrcode']);
        $this->assign('sysAvatar',$sysAvatar);
        $this->_setTab('avatar');
        $this->display();
    }
    ...
    public function index() {
        ...
        $aWeixin = I('post.weixin', '', 'op_t');
        $aSex = I('post.sex', 0, 'intval');
        $aEmail = I('post.email', '', 'op_t');
        $aSignature = I('post.signature', '', 'op_t');
        $aCommunity = I('post.pos_community', 0, 'intval');
        $aDistrict = I('post.pos_district', 0, 'intval');
...
//            $this->checkSignature($aSignature);
            ...
            $user['weixin'] = $aWeixin;
            $user['uid'] = get_uid();
            $user['catid'] = $this->_getSetUserCategory();
            $rs_member = D('Member')->save($user);

            //保存用户标签
            $rs_usertags = $this->_saveUserTags();
            //保存用户扩展信息
            $rs_expand = $this->_saveUserExpand();
            //设置字段是否显示 // TODO 不是在这里设置,这个是设置全局的,field表新增一个visiable
            $rs_show_field = $this->_saveUserFieldShow();
            //更新用户资料完善进度
            $rs_step = $this->_upUserStep();


            $ucuser['id'] = get_uid();
            $rs_ucmember = UCenterMember()->save($ucuser);
            clean_query_user_cache(get_uid(), array('nickname', 'catid', 'weixin', 'sex', 'signature', 'email', 'pos_province', 'pos_city', 'pos_district', 'pos_community'));

            //TODO tox 清空缓存
            S('weibo_at_who_users', null);
            S('weiquan_at_who_users', null);
            if ($rs_member || $rs_ucmember || $rs_usertags || $rs_expand) {
            ...
            //调用API获取基本信息
            $user = query_user(array('uid', 'title', 'level', 'score', 'username', 'nickname', 'weixin', 'signature', 'catid', 'email', 'mobile', 'avatar128', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 'pos_community'), $aUid);
            $user_field_sts = get_user_field_show($user['uid']);
            $user['category'] = D('UserCategory')->getById($user['catid']);
            $user['tag'] = D('Ucenter/UserTagLink')->getUserTag($aUid);
            $user['step'] = D('UserStep')->getStepPercentage($aUid);
            //echo json_encode(array('field'=>array('nickname'=>1,'mobile'=>1,'field_1'=>1,'field_10'=>1)));
            //显示页面
            $this->assign('user', $user);
            $this->assign('field_show', $user_field_sts); //print_r($user_field_sts);exit;

            $this->accountInfo();

            $this->assign('tab', $aTab);
            $this->getExpandInfo();
            $this->_getExpandInfoByUid($aUid);
            $this->_setTab('info');
            $this->display();
        }
    }
     ...
     * @return type
     */
    private function checkPostData(&$err = '') {
        $validator = new \FormValidator();
        $validator->addValidation("nickname", "minlen=2", L('_NICKNAME_FORM_ERROR_'));
        $validator->addValidation("weixin", "minlen=2", L('_WEIXIN_ACCOUNT_FORM_ERROR'));
        $validator->addValidation("wechat_type", "regexp=/^[0-9]{1}$/", L('_WECHAT_TYPE_FORM_ERROR_'));
        $validator->addValidation("cate_title", "minlen=2", L('_CATE_TITLE_FORM_ERROR_'));
       // $validator->addValidation("user_tag", "minlen=2", L('_USER_TAG_FORM_ERROR_'));
        $validator->addValidation("sex", "regexp=/^[0-9]{1}$/", L('_SEX_FORM_ERROR_'));
    ...
    private function _getExpandInfoByUid($uid) {
        $user_all = get_user_ext_info($uid); //print_r($user_all);exit;
        $this->assign('user_ext', $user_all);
    }
...
//                    $dl['create_time'] = $dl['update_time'] = time();
//                    if (!D('field')->add($dl)) {
...
//    }
    ...
    //获取、设置主推关键字
    private function _getSetUserCategory() {
        $cate_title = I('post.cate_title', '', 'op_t');
        $cate_title = trim($cate_title);
        $type = I('post.wechat_type', 1, 'intval');
        $result = D('UserCategory')->getByTitle($cate_title, $type);
        if ($result) {
            return $result['id'];
        }

        $id = D('UserCategory')->addCategory(array('title' => $cate_title, 'type' => $type, 'create_time' => time(), 'update_time' => time()));
        return $id;
    }

    //获取、设置用户其他关键字标签
    private function _saveUserTags() {
        $user_tag = I('post.user_tag', '', 'op_t');
        //$user_tag = str_replace("，", ',', $user_tag);
        //$tags = array();
        //list($tags[0], $tags[1], $tags[2], $tags[3]) = explode(',', $user_tag);
        $ids = array();
        foreach ($user_tag as $v) {
            $v=op_t($v);
            $ids[] = D('UserTag')->insertData(array('title' => trim($v), 'status' => 1));
        }
        $rs = NULL;
        if (!empty($ids)) {
            $rs = D('UserTagLink')->editData(implode(',', $ids));
        }
        return $rs;
    }

    //保存用户扩展信息
    private function _saveUserExpand() {
        $map_field['status'] = 1;
        $field_setting_list = D('FieldSetting')->getFieldList($map_field);
        ...
        foreach ($field_setting_list as $key => $val) {
            ...
            switch ($val['form_type']) {
                case 'input':
                    $val['value'] = op_t($_POST['field_' . $val['id']]);
                    if (!$val['value'] || $val['value'] == '') {
                        if ($val['required'] == 1) {
                            $this->error($val['field_name'] . L('_ERROR_CONTENT_NONE_') . L('_EXCLAMATION_'));
                        }
        ...
        foreach ($data as $dl) {
            ...
            if (!$res) {
                ...
                $dl['update_time'] = time();
                if (!D('Field')->saveData($dl, array('id' => $res['id']))) {
                    $this->error(L('_ERROR_INFO_CHANGE_') . L('_EXCLAMATION_'));
                }
                $is_success = true;
            }
...
//        }
    ...
    //保存用户设置显示字段
    private function _saveUserFieldShow() {
        $sts = array();
        $sts['mobile'] = I('post.mobile_show', 0, 'intval');
        $sts['sread_url'] = I('post.show_sread_url', 0, 'intval');
        $sts['diy_domain'] = I('post.show_diy_domain', 0, 'intval');
        $sts['field_1'] = I('post.show_field_1', 0, 'intval');
        $sts['field_10'] = I('post.show_field_10', 0, 'intval');
        $sts['field_11'] = I('post.show_field_11', 0, 'intval');
        $sts['field_12'] = I('post.show_field_12', 0, 'intval');
        $sts['field_13'] = I('post.show_field_13', 0, 'intval');
        $sts['field_14'] = I('post.show_field_14', 0, 'intval');
        $sts['field_15'] = I('post.show_field_15', 0, 'intval');
        $sts['field_16'] = I('post.show_field_16', 0, 'intval');

        $rs = D('UserFieldSetting')->saveData(array('uid' => is_login(), 'data' => json_encode(array('field' => $sts))));
        return $rs;
    }

    private function _upUserStep() {
        $user = query_user(array('avatar32', 'signature'), is_login());
        $_expand = D('Field')->getFieldByUid(is_login());
        $expand = array();
        foreach ($_expand as $v) {
            $expand[$v['field_id']] = $v;
        }

        $updata = array();
        $updata['uid'] = is_login();
        $updata['head'] = empty($user['avatar32']) ? 0 : 1;
        $updata['signature'] = empty($user['signature']) ? 0 : 1;
        $updata['qq'] = (isset($expand[1]['field_data']) && !empty($expand[1]['field_data'])) ? 1 : 0;
        $updata['weibo_url'] = (isset($expand[10]['field_data']) && !empty($expand[10]['field_data'])) ? 1 : 0;
        $updata['momo'] = (isset($expand[11]['field_data']) && !empty($expand[11]['field_data'])) ? 1 : 0;
        $updata['email'] = (isset($expand[12]['field_data']) && !empty($expand[12]['field_data'])) ? 1 : 0;
        $updata['hangye'] = (isset($expand[13]['field_data']) && !empty($expand[13]['field_data'])) ? 1 : 0;
        $updata['zhiye'] = (isset($expand[14]['field_data']) && !empty($expand[14]['field_data'])) ? 1 : 0;
        $updata['techang'] = (isset($expand[15]['field_data']) && !empty($expand[15]['field_data'])) ? 1 : 0;
        $updata['xingqu'] = (isset($expand[16]['field_data']) && !empty($expand[16]['field_data'])) ? 1 : 0;
        $updata['wei_url'] = (isset($expand[17]['field_data']) && !empty($expand[17]['field_data'])) ? 1 : 0;
        $updata['wei_addr'] = (isset($expand[18]['field_data']) && !empty($expand[18]['field_data'])) ? 1 : 0;

        $rs = D('UserStep')->saveData($updata);
    }
    public function accountManagemen(){
        $userInfo=query_user(array('mobile','password'),is_login());
        
        if(!$userInfo['mobile']){
            $this->redirect ( 'Config/bindMobile' );
        }
        elseif(!$userInfo['password']){
            $this->redirect ( 'Config/setPassword' );
        }
        $this->assign('mobile',$userInfo['mobile']);
        $this->display();
    }
    public function bindMobile(){
        if(query_user(array('mobile'),is_login())['mobile']){
            $this->error('您已绑定手机');
         }
      if(IS_POST){   
          if (!D('Verify')->checkVerify(I('post.mobile'), 'mobile', I('post.reg_verify'), 0)) {
              $this->error('手机号验证失败');
          }
          M('UcenterMember')->where(array('id'=>is_login()))->save(array('mobile'=>I('post.mobile')));
         // $this->redirect ( 'Config/setPassword' );
         $this->ajaxReturn(array('status'=>1,'info'=>'绑定手机成功'));
      }
       else{      
    ...
    public function setPassword(){
        ...
        if(IS_POST){
            ...
            if(!$userInfo['password']){
               if(D('User/UcenterMember')->setPassword($pass)){
                   $this->ajaxReturn(array('status'=>1,'info'=>'密码设置成功'));
               }
                   $this->ajaxReturn(array('status'=>0,'info'=>'密码设置失败，重新操作'));
            }
            else{
                $oldPassword=I('post.oldpassword');
                if($oldPassword==$pass){
                    $this->ajaxReturn(array('status'=>0,'info'=>'新密码和旧密码不能相同'));
                }
                if(D('User/UcenterMember')->changePassword($oldPassword,$pass)){
                    $this->ajaxReturn(array('status'=>1,'info'=>'密码修改成功'));
                }
                $this->ajaxReturn(array('status'=>1,'info'=>'密码修改失败，请重试!'));
            }
        }else{
            if($userInfo['password']){
