...
class IndexController extends BaseController
{
	...
	{
		$output = [];
		$tab_config = get_kanban_config('WEIQUAN_DEFAULT_TAB', 'enable', 
			array(
					'all', 'related'
			));
		if(! is_login())
		...
		// 查询
		$output['list'] = $weiquanModel->getWeiquanList($param);
		$output['total_count'] = $weiquanModel->getWeiquanCount($param['where']);
		
		// 获取置顶微博
		$output['top_list'] = $weiquanModel->getWeiquanList(
			array(
					'where' => array(
							'status' => 1, 'is_top' => 1
					)
		...
		$output['loadMoreUrl'] = U('loadweiquan', 
			array(
					'uid' => $uid, 'su' => $show_user
			));
		...
		$output['page'] = $aPage;
		$output['user_info'] = get_user_info($uid);
		$this->assign($output);
		$this->display();
	}
	 ...
	 */
	private function assignSelf($uid)
	{
		$self = query_user(
			array(
					'title', 'avatar128', 'nickname', 'uid', 'space_url', 'score', 'title', 'fans', 'following', 
					'weibocount', 'rank_link'
			), $uid);
		// 获取用户封面id
	...
	private function userInfo($uid = null)
	{
		$user_info = query_user(
			array(
					'avatar128', 'nickname', 'uid', 'space_url', 'score', 'title', 'fans', 'following', 'weibocount', 
					'rank_link', 'signature'
			), $uid);
		// 获取用户封面id
	...
	public function information($uid = 0)
	{
		// 调用API获取基本信息
		// TODO tox 获取省市区数据
		$user = query_user(
			array(
					'nickname', 'signature', 'mobile', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 
					'pos_community'
			), $uid);
		if($user['pos_province'] != 0)
		{
			$user['pos_province'] = D('district')->where(
				array(
						'id' => $user['pos_province']
				))->getField('name');
			$user['pos_city'] = D('district')->where(
				array(
						'id' => $user['pos_city']
				))->getField('name');
			$user['pos_district'] = D('district')->where(
				array(
						'id' => $user['pos_district']
				))->getField('name');
			$user['pos_community'] = D('district')->where(
				array(
						'id' => $user['pos_community']
				))->getField('name');
		}
		// 显示页面
	...
	public function _profile_group_list($uid = null)
	{
		...
		if($fields_list)
		{
			$fields_group_ids = D('FieldSetting')->where(
				array(
						'id' => array(
								'in', $fields_list
						), 'status' => '1'
			...
			if($fields_group_ids)
			{
				...
				$map['status'] = 1;
				$profile_group_list = D('field_group')->where($map)
					->order('sort asc')
					->select();
			}
		}
	...
	public function _info_list($id = null, $uid = null)
	{
		...
		{
			// 查看别人的扩展信息
			$field_setting_list = D('field_setting')->where(
				array(
						'profile_group_id' => $id, 'status' => '1', 'visiable' => '1', 
						'id' => array(
								'in', $fields_list
						)
				))
	...
	public function _get_field_data($data = null)
	{
		...
		switch($data['form_type'])
		{
			...
			case 'select':
				...
				break;
			case 'checkbox':
				$result['field_data'] = isset($data['field_content']['field_data']) ? implode(' ', 
					explode('|', $data['field_content']['field_data'])) : "还未设置";
				break;
			case 'time':
				$result['field_data'] = isset($data['field_content']['field_data']) ? date("Y-m-d", 
					$data['field_content']['field_data']) : "还未设置";
				break;
		}
	...
	public function appList($uid = null, $page = 1, $tab = null)
	{
		...
		if(! $appArr)
		{
			$this->redirect('Usercenter/Index/information', 
				array(
						'uid' => $uid
				));
		}
	...
	public function _tab_menu()
	{
		...
		foreach($modules as $m)
		{
			...
			{
				if(file_exists(
					APP_PATH . $m['name'] . '/Widget/UcenterBlockWidget.class.php'))
				{
					$apps[] = array(
							'data-id' => $m['name'], 'title' => $m['alias'], 'sort' => $m['sort'], 
		...
		$show_tab = get_kanban_config('UCENTER_KANBAN', 'enable', '', 'USERCONFIG');
		$apps[] = array(
				'data-id' => 'info', 'sort' => '0', 'title' => '资料', 'key' => 'info'
		);
		$apps[] = array(
				'data-id' => 'rank_title', 'sort' => '0', 'title' => L('_RANK_TITLE_'), 'key' => 'rank_title'
		);
		$apps[] = array(
				'data-id' => 'follow', 'sort' => '0', 'title' => L('_FOLLOWERS_NO_SPACE_') . '/粉丝', 'key' => 'follow'
		);
	...
	public function _fans_and_following($uid = null)
	{
		...
		// 我的粉丝展示
		$map['follow_who'] = $uid;
		$fans_default = D('Follow')->where($map)
			->field('who_follow')
		...
		foreach($fans_default as &$user)
		{
			$user['user'] = query_user(
				array(
						'avatar64', 'uid', 'nickname', 'fans', 'following', 'weibocount', 'space_url', 'title'
				), $user['who_follow']);
		}
		unset($user);
		...
		$follow_default = D('Follow')->where($map_follow)
			->field('follow_who')
			->order('create_time desc')
			->limit(8)
			->select();
	...
	public function fans($uid = null, $p = 1)
	{
		$uid = isset($uid) ? $uid : is_login();
		$isOwner = false; // 是不是自己的粉丝
		$isFans = false; // 是不是我的粉丝
		$uid == is_login() && $isFans = $isOwner = true;
		$this->assign('tab', 'fans');
		$name = op_t(I('get.name'));
		$sort = op_t(I('get.sort'));
		$param = array();
		$param['page'] = $p;
		$name && $param['where']['nickname'] = $name;
		list ($param['where']['sort'], $order, $by) = self::fansFollowSort($sort);
		$order && $this->assign('by', $by == 'asc' ? 'desc' : 'asc');
		$this->assign('order', $order);
		// $sort&&$param['where']['sort']=$sort;
		// $name&&$param['name']=$param;
		$togtherFollow = intval(I('get.togtherFollow'));
		if($togtherFollow)
		{
			if(! $isOwner)
			{
				$fansData = D('Follow')->getTogtherFollow($uid, $param);
				$this->assign('togtherFollowTotal', $following['total']);
			}
		}
		else
		{
			$fansData = D('Fans')->getFans($uid, $param, $isOwner);
			$this->assign('togtherFollowTotal', D('Follow')->getTogtherFollow($uid, $param, true));
		}
		// print_r($fansData);die;
		if($fansData['list'])
		{
			foreach($fansData['list'] as $k => $v)
			{
				$fansData['fansInfo'][$k] = query_user(
					array(
							'pos_city', 'pos_province', 'pos_district', 'pos_community', 'weixin', 'avatar128', 'uid', 
							'nickname', 'fans', 'following', 'weibocount', 'title', 'level', 'username', 'catid', 'ipai'
					), $v);
				if($fansData['fansInfo'][$k]['pos_province'] != 0)
				{
					$fansData['fansInfo'][$k]['pos_province'] = D('District')->getNameById(
						$fansData['fansInfo'][$k]['pos_province']);
					$fansData['fansInfo'][$k]['pos_city'] = D('District')->getNameById(
						$fansData['fansInfo'][$k]['pos_city']);
					$fansData['fansInfo'][$k]['pos_district'] = D('District')->getNameById(
						$fansData['fansInfo'][$k]['pos_district']);
					$fansData['fansInfo'][$k]['pos_community'] = D('District')->getNameById(
						$fansData['fansInfo'][$k]['pos_community']);
				}
				$fansData['fansInfo'][$k]['followFlag'] = $this->getFollowFlag(is_login(), $v, $isFans);
			}
		}
		$userInfo = query_user(
			array(
					'sex', 'following', 'fans', 'uid','username'
			), $uid);
		$this->assign('_page', getPageLinkUc($fansData['total'], 10));
		$this->assign('userInfo', $userInfo);
		$this->assign('fans', $fansData);
		// $this->assign('totalCount', $fansData['total']);
		$this->assign('uid', $uid);
		// 四处一词 seo
		$str = '{$user_info.nickname|op_t}';
		...
		$this->assign('followFlag', R('Ucenter/index/getFollowFlag', array(
				is_login(), $uid
		)));
		// 四处一词 seo end
		$this->display();
	...
	public static function fansFollowSort($sort)
	{
		$sortStr = '';
		list ($order, $by) = @explode('-', $sort);
		if(! $order || ! $by || ! in_array($by = strtolower($by), array(
				'asc', 'desc'
		)))
		{
			return array(
					"create_time desc", '', 'desc'
			);
		}
		switch($order)
		{
	...
	public function followAction()
	{
		...
		switch($flag)
		{
			...
			case 2: // 取消关注
				...
				break;
			case 3: // 移除粉丝
				$r2 = D('Fans')->delFans($uid, is_login()); // 删除粉丝
				break;
			default:
				break;
		}
	...
	public function getFollowFlag($uid, $fansUid, $isFans = false, $isFollow = false)
	{
		$followFlag = 0; // 0两者未关注 1我关注2被关注3相互关注(双方都关注)
		if(! $uid)
		{
			return $followFlag;
		}
		if($isFans)
		{
			if(D('Follow')->isFollow($uid, $fansUid))
			{
				$followFlag = 3;
			}
			else
			...
			if($isFollow)
			{
				if(D('Follow')->isFollow($fansUid, $uid))
				{
					$followFlag = 3;
				}
				else
			...
			{
				$isFollow = D('Follow')->isFollow($uid, $fansUid);
				$isFans = D('Follow')->isFollow($fansUid, $uid);
				switch(true)
				{
					case ! $isFollow && ! $isFans:
						$followFlag = 0;
						break;
					case $isFollow && ! $isFans:
						$followFlag = 1;
	...
	public function following($uid = null, $p = 1)
	{
		$uid = isset($uid) ? $uid : is_login();
		$isOwner = false; // 是不是自己
		$isFollow = false; // 是不是我关注的
		$uid == is_login() && $isOwner = $isFollow = true;
		// $this->assign('tab', 'fans');
		$name = op_t(I('get.name'));
		$sort = op_t(I('get.sort'));
		$param = array();
		...
		if($togtherFollow)
		{
			if(! $isOwner)
			{
				$following = D('Follow')->getTogtherFollow($uid, $param);
				$this->assign('togtherFollowTotal', $following['total']);
			}
		}
		else
		{
			$following = D('Follow')->getFollowList($uid, $param, $isOwner);
			$this->assign('togtherFollowTotal', D('Follow')->getTogtherFollow($uid, $param, true));
		}
		// $following['list']=array_filter($following['list']);
		// dump($following['list']);die;
		if($following['list'])
		{
			foreach($following['list'] as $k => $v)
			{
				$following['followInfo'][$k] = query_user(
					array(
							'pos_city', 'pos_province', 'pos_district', 'pos_community', 'weixin', 'avatar128', 'uid', 
							'nickname', 'fans', 'following', 'weibocount', 'weiquanRemark', 'title', 'level', 'username', 
							'catid', 'ipai'
					), $v);
				if($following['followInfo'][$k]['pos_province'] != 0)
				{
					$following['followInfo'][$k]['pos_province'] = D('District')->getNameById(
						$following['followInfo'][$k]['pos_province']);
					$following['followInfo'][$k]['pos_city'] = D('District')->getNameById(
						$following['followInfo'][$k]['pos_city']);
					$following['followInfo'][$k]['pos_district'] = D('District')->getNameById(
						$following['followInfo'][$k]['pos_district']);
					$following['followInfo'][$k]['pos_community'] = D('District')->getNameById(
						$following['followInfo'][$k]['pos_community']);
				}
				$following['followInfo'][$k]['followFlag'] = $this->getFollowFlag(is_login(), $v, false, $isFollow);
			}
		}
		// dump($fansData['fansInfo']);die;
		$userInfo = query_user(
			array(
					'sex', 'following', 'fans', 'uid','username'
			), $uid);
		$this->assign('_page', getPageLinkUc($following['total'], 10));
	...
	public function rank($uid = null)
	{
		...
		$rankList = D('rank_user')->where(
			array(
					'uid' => $uid, 'status' => 1
			))
			->field('rank_id,reason,create_time')
			->select();
		foreach($rankList as &$val)
		{
			$rank = D('rank')->where('id=' . $val['rank_id'])->find();
			$val['title'] = $rank['title'];
			$val['logo_url'] = get_pic_src(
				M('picture')->where('id=' . $rank['logo'])
					->field('path')
					->getField('path'));
	...
	public function rankVerifyFailure()
	{
		...
		$rankList = D('rank_user')->where(
			...
			->field('id,rank_id,reason,create_time')
			->select();
		foreach($rankList as &$val)
		{
			$rank = D('rank')->where('id=' . $val['rank_id'])->find();
	...
	public function rankVerifyCancel($rank_id = null)
	{
		...
		if(is_login() && $rank_id)
		{
			...
			if($result)
			{
				D('Message')->sendMessageWithoutCheckSelf(is_login(), L('_MESSAGE_RANK_CANCEL_1_'), 
					L('_MESSAGE_RANK_CANCEL_2_'), 'Ucenter/Message/message', 
					array(
							'tab' => 'system'
					));
	...
	public function rankVerify($rank_user_id = null)
	{
		...
		if($rank_user_id)
		{
			...
			$map_already['id'] = array(
					'neq', $rank_user_id
			);
			D('Message')->sendMessageWithoutCheckSelf(is_login(), L(''), L(''), 'Ucenter/Message/message', 
				array(
						'tab' => 'system'
				));
		}
		$alreadyRank = D('rank_user')->where($map_already)
			->field('rank_id')
			->select();
	...
	public function verify($rank_id = null, $reason = null, $rank_user_id = 0)
	{
		...
		if($result)
		{
			D('Message')->sendMessageWithoutCheckSelf(is_login(), L('_MESSAGE_RANK_APPLY_1_'), 
				L('_MESSAGE_RANK_APPLY_2_'), 'Ucenter/Message/message', 
				array(
						'tab' => 'system'
				));
