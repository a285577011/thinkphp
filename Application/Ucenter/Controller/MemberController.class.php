...
class MemberController extends Controller
{
    ...
    public function register()
    {
        if (IS_POST) {
            ...
            $type = I('get.type', '', 'op_t');
            $role = 1;
            $aUnType = 3; // 手机注册
            
            if($type == 'weixin'){ // 微信注册
                $session = session('SYNCLOGIN');
                if(!$session['TYPE'] || !$session['OPENID']){
                    $this->error('TOKEN已过期，请重新扫描注册', addons_url('SyncLogin://Base/login',array('type'=>'weixin')));
                }
                /* 注册用户 */
                $post['openid'] = $session['OPENID'];
                $ucenterMemberModel=UCenterMember();
                $uid = $ucenterMemberModel->registerMutil($post, 2);
                if (0 < $uid) { //注册成功                
                    $uid = $ucenterMemberModel->login($post['username'], $post['password'], $aUnType); //通过账号密码取到uid
                    D('Member')->login($uid, false, $role); //登陆
            ...
            if (!D('Verify')->checkVerify($post['mobile'], 'mobile', $post['reg_verify'], 0)) {
                $this->error('手机号验证失败', U('register@'));
            }
            ...
            switch($step){
                ...
                case 2:
                    ...
                    $cat['type'] = $post['type'];
                    $cat['type'] == 2 ? $cat['title'] = $post['keyword_ng'] : $cat['title'] = $post['keyword'];
                    if(!$cat = $userCategoryModel->create($cat)){
                    ...
                    /* 注册用户 */
                    $ucenterMemberModel=UCenterMember();
                    $uid =$ucenterMemberModel ->register($post['username'], $post['nickname'], $post['password'], $post['mobile'], $post['weixin'], $aUnType, $post['sex'], $catid);
                    if (0 < $uid) { //注册成功
                        //$this->initInviteUser($uid, $aCode, $role); //邀请注册
                        $ucenterMemberModel->initRoleUser($role, $uid); //初始化角色用户
            ...
            $type = I('get.type', '', 'op_t');
            $sync = I('get.sync', '', 'op_t');
            $this->assign('step', $step);
            $this->assign('type', $type == '' ? 'username' : $type);
            if($type == 'weixin'){ // 微信登录、注册
                $session = session('SYNCLOGIN');
                if(!$session['TYPE'] || !$session['OPENID']){
                    $this->error('TOKEN已过期，请重新扫描注册', addons_url('SyncLogin://Base/login',array('type'=>'weixin')));
                }
                $uid = D('sync_login')->where(array('type_uid' => $session['OPENID'], 'type' => $session['TYPE']))->getField('uid');
                if(!$uid){
                    $this->error('微信未同步，请重新扫描注册', addons_url('SyncLogin://Base/login',array('type'=>'weixin')));
                }
                $userinfo = query_user(array('username'), $uid);
                if($userinfo['username']){
                    // 自动登录
                    $rs = D('Member')->login($uid, false); //登陆
                    if ($rs) {
                        session('SYNCLOGIN', null);
                        redirect(U('@'));
                    }
                }
                $this->display('sync_register');
            }else{
    ...
    {
        $step = I('get.step', '', 'op_t');
        $uid = session('temp_login_uid');
        $roleId = session('temp_login_role_id');
        if (empty($uid)) {
        ...
        if (get_next_step($step) != $step) {
            $step = check_step($step);
            $_GET['step'] = $step;
            $userRoleModel->where($map)->setField('step', $step);
        }
        $userRoleModel->where($map)->setField('step', $step);
        if ($step == 'finish') {
            D('Member')->login($uid, false, $roleId);
        }
        $this->assign('step', $step);
        ...
        if (IS_POST) {
            ...
            if ($invite) {
                if ($invite['end_time'] > time()) {
                    $result['status'] = 1;
                    $result['url'] = U('Ucenter/Member/register', array('code' => $aCode, 'type' => $type));
                } else {
    ...
    public function upRole()
    {
        ...
        if (IS_POST) {
            $uid = is_login();
            $result['status'] = 0;
            if ($uid > 0 && $roleId != get_login_role()) {
                $aCode = I('post.code', '', 'op_t');
                if (!mb_strlen($aCode)) {
                    $result['info'] = L('_INFO_PLEASE_INPUT_').L('_EXCLAMATION_');
                    $this->ajaxReturn($result);
        ...
        switch ($step){
            case 1:
                if (IS_POST) { //登录验证
            		...
            		$uid = D('User/UcenterMember')->getUidByMobile($mobile);
            		if(!$uid){
            			$this->error('手机号未注册');
            		}
            		
            		// 发送手机验证码
                    if(I('action', '', 'op_t') == 'sendverify'){
                        
                        $time = time();
                        $resend_time =  modC('SMS_RESEND','60','USERCONFIG');
                        if($time <= session('verify_time')+$resend_time ){
                            $this->error('请'.($resend_time-($time-session('verify_time'))).'秒后再发送');
                        }
                        ...
                        $res =  A('Member')->doSendVerify($mobile, $verify, 'mobile');
                        if ($res === true) {
                            session('verify_time',$time);
                            if(is_array($verify)){
                                $this->success($verify['code']);
                            }else{
            ...
            case 2:
                ...
                $isVerify=D('Common/Verify')->checkVerify($mobile, $type='mobile', $verify, 0);
                if(!$isVerify){
            ...
            case 3:
                ...
                if($isVerify){
                    ...
                    $uid = $ucModel->getUidByMobile( $mobile );
                    if (!$uid) {
                        $this->error('手机号未注册');
                    }
                    $res = $ucModel->where(array('id'=>$uid,'status'=>1))->save(array('password' =>think_ucenter_md5($password, UC_AUTH_KEY)));
                    if (!$res) {
                        $this->error('重置密码失败');
                    }
                }else{
    ...
    public function doSendVerify($account, $verify, $type)
    {
        switch ($type) {
            case 'mobile':
                ...
                $res = sendSMS($account, $content, $id);
                return $res;
                break;
            case 'email':
                //发送验证邮箱
    ...
    public function activate()
    {
        ...
        $uid = session('temp_login_uid');
        $status = UCenterMember()->where(array('id' => $uid))->getField('status');
        if ($status != 3) {
            redirect(U('ucenter/member/login'));
        }
        $info = query_user(array('uid', 'nickname', 'email'), $uid);
        $this->assign($info);
        $this->display();
    }
    ...
    public function changeEmail()
    {
        ...
        $uid = session('temp_login_uid');
        $ucenterMemberModel = UCenterMember();
        $ucenterMemberModel->where(array('id' => $uid))->getField('status');
        if ($ucenterMemberModel->where(array('id' => $uid))->getField('status') != 3) {
            $this->error(L('_ERROR_AUTHORITY_LACK_').L('_EXCLAMATION_'));
        }
        $ucenterMemberModel->where(array('id' => $uid))->setField('email', $aEmail);
        clean_query_user_cache($uid, 'email');
        $res = $this->activateVerify();
    ...
    public function saveAvatar()
    {
        ...
        $data = array('uid' => $uid, 'path' => $returnPath);
        $res = D('Ucenter/Avatar')->saveAvatar($data);
        if (!$res) {
            $this->error(L('_FAIL_ACTIVE_').L('_EXCLAMATION_').D('Ucenter/Avatar')->getError());
        }
    ...
    public function doActivate()
    {
        ...
        $type = I('get.type', '', 'op_t');
        $uid = I('get.uid', 0, 'intval');
        $check = D('Verify')->checkVerify($aAccount, $type, $aVerify, $uid);
        if ($check) {
...
//                    $this->error(L('_ERROR_EMAIL_LENGTH_LIMIT_'));
    ...
    {
        $roleId = I('post.role_id', 0, 'intval');
        $uid = is_login();
        $data['status'] = 0;
        if ($uid && $roleId != get_login_role()) {
            $roleUser = D('UserRole')->where(array('uid' => $uid, 'role_id' => $roleId))->find();
            if ($roleUser) {
                $memberModel = D('Common/Member');
                $memberModel->logout();
                clean_query_user_cache($uid, array('avatar64', 'avatar128', 'avatar32', 'avatar256', 'avatar512', 'rank_link'));
                $result = $memberModel->login($uid, false, $roleId);
                if ($result) {
    ...
    public function verifyUser(){
        $isTrue=D('User/UcenterMember')->verifyUser(is_login(),I('post.password'));
        if($isTrue){
            $this->ajaxReturn(array('status'=>1,'info'=>'密码验证成功'));
        }
