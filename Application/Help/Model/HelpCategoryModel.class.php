...
class HelpCategoryModel extends Model{
    ...
    {
        $list = $this->where($map)->field('id,title,pid,sort,status')->order('pid asc,sort asc')->select();
        if (!$type) {
            return $list;
        }
        $tree = D('Tree')->toTree($list, 'id', 'pid', '_child');
        //dump(D('Tree')->toFormatTree($tree, 'title', 'id', 'pid'));
        return D('Tree')->toFormatTree($tree, 'title', 'id', 'pid');
