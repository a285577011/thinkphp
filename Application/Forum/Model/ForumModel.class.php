...
class ForumModel extends Model
{
    ...
    public function getForumList ($map_type = array('status' => 1))
    {
        ...
        if (empty($forum_list)) {
            // 读取板块列表
            $forum_list = D('Forum/Forum')->where($map_type)
                ->order('sort asc')
                ->select();
            ...
            foreach ($forum_list as &$f) {
                $map['status'] = 1;
                $map['forum_id'] = $f['id'];
                $f['background'] = intval($f['background']) != 0 ? getThumbImageById(
                        $f['background'], 1160, 180) : C(
                        'TMPL_PARSE_STRING.__IMG__') . '/default_head.jpg';
                $f['logo'] = intval($f['logo']) != 0 ? getThumbImageById(
                        $f['logo'], 128, 128) : C('TMPL_PARSE_STRING.__IMG__') .
                         '/default_logo.jpg';
                ...
                $f['total_count'] = $f['topic_count'] +
                         $forumPostReplyModel->where($map)->count();
            }
            unset($f);
    ...
    {
        $forum_ids = M('ForumFollow')->field('forum_id')
            ->where(array(
                'uid' => $uid
        ))
            ->select();
