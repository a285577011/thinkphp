...
class ForumPostReplyModel extends Model
{
    ...
    public function addReply ($post_id, $content)
    {
        ...
        $pageCount = $this->sendReplyMessage(is_login(), $post_id, $content, 
                $result);
        
        $this->handleAt($content, 'Forum/Index/detail#' . $result, 
                array(
                        'id' => $post_id,
                        'page' => $pageCount
                ));
    ...
    private function sendReplyMessage ($uid, $post_id, $content, $reply_id)
    {
        ...
        $content = L('_REPLY_CONTENT_WITH_COLON_') .
                 mb_substr(op_t($content), 0, 20);
        D('Message')->sendMessage($post['uid'], $title, $content, 
                'Forum/Index/detail#' . $reply_id, 
                array(
                        'id' => $post_id,
                        'page' => $pageCount
                ), $uid, 2);
    ...
    public function getReplyList ($map, $order, $page, $limit)
    {
        ...
        if ($replyList == null) {
            $replyList = D('ForumPostReply')->where($map)
                ->order($order)
                ->select();
            foreach ($replyList as &$reply) {
                $reply['user'] = query_user(
                        array(
                                'avatar128',
                                'nickname',
                                'space_url',
                ...
                $reply['lzl_count'] = D('forum_lzl_reply')->where(
                        'is_del=0 and to_f_reply_id=' . $reply['id'])->count();
            }
            unset($reply);
    ...
    public function delPostReply ($id)
    {
        ...
        if ($res) {
            $lzlReply_idlist = D('ForumLzlReply')->where(
                    'is_del=0 and to_f_reply_id=' . $id)
                ->field('id')
                ->select();
            $info['is_del'] = 1;
