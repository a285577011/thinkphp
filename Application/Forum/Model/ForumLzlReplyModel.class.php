...
class ForumLzlReplyModel extends Model
{
    ...
    public function addLZLReply ($post_id, $to_f_reply_id, $to_reply_id, $to_uid, 
            $content, $p, $send_message = true)
    {
        // 新增一条回复
        $data = array(
                'uid' => is_login(),
            ...
            $to_f_reply_id, $result, $p)
    {
        ...
        D('Message')->sendMessage($to_uid, $title, $content, 
                'Forum/Index/detail#' . $to_f_reply_id, 
                array(
                        'id' => $post_id,
                        'page' => $p,
                        'sr' => $to_f_reply_id,
    ...
    public function getLZLReplyList ($to_f_reply_id, $order, $page = 1, $limit)
    {
        ...
        if ($list == null) {
            $list = D('forum_lzl_reply')->where(
                    'is_del=0 and to_f_reply_id=' . $to_f_reply_id)
                ->order($order)
                ->select();
            foreach ($list as $k => &$v) {
                $v['userInfo'] = query_user(
                        array(
                                'avatar128',
                                'nickname',
                                'uid',
                ...
                $v['content'] = parse_expression(op_t($v['content']));
            }
            unset($v);
    ...
    private function handleAt ($post_id, $to_f_reply_id, $content, $p, $map)
    {
        ...
        $pageCount = ceil($count / $limit);
        // 增加微博的评论数量
        D('ContentHandler')->handleAtWho($content, 
                'Forum/Index/detail#' . $to_f_reply_id, 
                array(
                        'id' => $post_id,
                        'page' => $p,
