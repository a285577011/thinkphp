...
class ForumMessageModel extends Model implements IMessage
{
    ...
    public function getSource ($message)
    {
        if ($message['apptype'] == 'reply') {
            ...
            $source['source_url'] = U('Forum/Index/detail', 
                    array(
                            'id' => $post['id']
                    ));
            $source['title'] = L('_BASED_ON_') . $post['title'] .
                     L('_THE_PASTE_INSIDE_DIALOGUE_');
            ...
            $source['title'] = L('_BASED_ON_') . $post['title'] . L(
                    '_DIALOGUE_');
        }
    ...
    public function postMessage ($source_message, $talk, $content)
    {
        ...
        foreach ($uids as $uid) {
            if ($uid != is_login()) {
                ...
                $lzlReplys[] = D('Forum/ForumLzlReply')->addLZLReply(
                        $source_message['source_id'], $source_message['find_id'], 
                        $source_message['find_id'], $uid, 
                        L('_REPLY_') . $user['username'] . '： ' . $content, 
                        false);
            }
        }
