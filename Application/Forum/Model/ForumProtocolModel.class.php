...
class ForumProtocolModel extends Model
{
    ...
    public function profileContent ($uid = null, $page = 1, $count = 15, $tab = null)
    {
        ...
        if ($tab == 'forum') {
            $map['status'] = 1;
            $result = $this->forum_postModel->where($map)
                ->page($page, $count)
            ...
            unset($map_in['uid']);
            $map_in['status'] = 1;
            $result = $this->forum_postModel->where($map_in)
                ->page($page, $count)
                ->order('update_time desc')
                ->select();
            foreach ($result as &$v) {
        ...
        $content = '';
        $content = $view->fetch(T('Application://Forum@Index/profile_content'), 
                $content);
        return $content;
    }
    ...
    private function _getForumList ()
    {
        ...
        if (empty($forum_list)) {
            // 读取板块列表
            $forum_list = D('Forum/Forum')->where(
                    array(
                            'status' => 1
                    ))
                ->order('sort asc')
    ...
    private function _getInMap ($map = array())
    {
        $map_reply = $map;
        $map_reply['status'] = 1;
        $reply_ids = $this->forum_replyModel->where($map_reply)
            ->field('post_id')
            ->select();
        $reply_ids = array_column($reply_ids, 'post_id');
        $map_lzl_reply = $map;
        $map_lzl_reply['is_del'] = 0;
        $lzl_reply_ids = $this->forum_lzl_replyModel->where($map_lzl_reply)
            ->field('post_id')
            ->select();
