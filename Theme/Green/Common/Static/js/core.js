     ...
     'bind_follow': function () {
        ...
        $('[data-role="follow"]').click(function () {
            ...
            $.post(U('Core/Public/follow@'), {uid: uid,cookie:phpSessionId}, function (msg) {
                if (msg.status == "1") {
                    $this.removeClass("attention attention2 attention3 border").addClass('attention2 border');
                    $this.attr('data-role', 'unfollow');
                    $this.find('p').html(msg.btn_text);
                    follower.bind_follow();                   
                } else if (msg.status == '2') {
                    $this.removeClass("attention attention2 attention3 border").addClass('attention3 border');
                    $this.attr('data-role', 'unfollow');
                    $this.find('p').html(msg.btn_text);
                    follower.bind_follow();
                } else {
        ...
        $('[data-role="unfollow"]').click(function () {
            ...
            $.post(U('Core/Public/unfollow@'), {uid: uid,cookie:phpSessionId}, function (msg) {
                if (msg.status) {
                   $this.removeClass("attention attention2 attention3 border").addClass('attention');
...
function bind_like() {
    ...
    $('.like_btn').click(function () {       
        ...
        if($(me).find(".icon-heart-empty").length>0){            
            $.post(LIKE_ADD_URL, {weiquan_id: wid}, function (msg) {
                if (msg.status==1) {  
                    debugger;
                    $(me).parent().find("._like_user").html(msg.html);                    
                    $(me).find('i').eq(0).removeClass().addClass('icon-heart');
                    $(me).find('._lng').html(msg.lng_cancel);
                    toast.success(msg.info, '温馨提示');
                } else {
                    toast.error(msg.info, '温馨提示');
                }
          ...
          $.post(LIKE_DEL_URL, {weiquan_id: wid}, function (msg) {
                if (msg.status==1) {  
                    $(me).parent().find("._like_user").html(msg.html); 
                    $(me).find('i').eq(0).removeClass().addClass('icon-heart-empty');
                    $(me).find('._lng').html(msg.lng_like);
                    toast.success(msg.info, '温馨提示');
                } else {
                    toast.error(msg.info, '温馨提示');
                }
...
$(document).click(function(e){
	...
	if($(obj).closest("#emot_content").length==0&&$(obj).closest(".getSmile").length==0) {
		$(".XT_face").remove();
	}
});
