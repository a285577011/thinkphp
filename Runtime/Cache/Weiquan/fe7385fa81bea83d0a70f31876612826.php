...
<div class="nav_box">
    ...
    <?php if($type == 'all'): ?><div class="right_body left">        
          <?php if($type=='all'){ ?>
    <div class="release_box cl50 weibo_content weibo_post_box">
                ...
                <textarea id="weibo_content" class="release px14 left"  placeholder="<?php echo L('_PLACEHOLDER_SOMETHING_TO_WRITE_');?>" onfocus="startCheckNum_quick($(this))" onblur="endCheckNum_quick()"></textarea>
                               <a onclick="insertFace($(this))">
                <div class="smile_box left getSmile">
                    <div class="smile left"></div>
                    <p class="left"><?php echo L('_EMOT_');?></p>
                </div>
                </a>
                <div class="picture_box left" id="insert_image" onclick="insert_image.insertImage(this)">
                ...
                <input class="release_button hand right px14" id="release_button" type="submit" value="<?php echo L('_PUBLISH_');?>"  data-role="send_weiquan" data-url="<?php echo U('Index/doSend');?>"/>
                <div class="clear"></div> 
...
<?php } ?>
            <div class="trends_box">
             <?php switch($type): case "related": ?><ul class="trends_title cl50" id="weibo_filter">                                
                            <li class="wechat_place left" id="<?php echo ($tab); ?>" data-url="<?php echo U('Weiquan/Index/index',array('type'=>related));?>"><?php echo L('_MY_RELATED_');?></li>
                        </ul><?php break; endswitch;?> 
                <script>
                ...
                <?php if($total_count == 0): ?><div class="dynanicimg_box">
                    ...
                    <?php if($page==1&&$isMore){ ?>
                      ...
                      <?php }elseif($page>1){ ?>
                        ...
                        <div class="clear"></div>
                    <?php } ?>
                    <div id="index_weibo_page" <?php if($page==1){ ?>style="display:none" <?php } ?>>
...
<?php if($type == 'all'): ?><!--左边的改版后的悬浮的个人信息-->
<style>
h2,p{
padding:0;
margin:0;
 ...
 <div class="left_body right ">
 			<div class="mydata_box">
 				<a href="<?php echo U('Weiquan/Index/index',array('type'=>related));?>"><img class="my_dataimg left" src="<?php echo ($user_info["avatar"]); ?>" alt="<?php echo ($user_info["nickname"]); ?>"/></a>
 				<ul class="mydata_right right">
 					<li>
 						<span class="mydata_name bold">        <?php if($user_info['nickname'] == ''): echo L('_NO_IDEA_');?>
            <?php else: ?>
            <a href="<?php echo U('Weiquan/Index/index',array('type'=>related));?>"><?php echo ($user_info["nickname"]); ?></a><?php endif; ?></span>
                   <?php if($user_info['sex']==2){ ?><span class="head_sex"> </span><?php }elseif($user_info['sex']==1){ ?>
             <span class="head_sex2"></span>
            <?php } ?>
 						<span class="grade<?php if($user_info['type'] == 1): ?>2<?php endif; ?> px12"><i>Lv.<?php echo ($user_info["level"]); ?></i></span>
 						<!--<span class="grade2 px12"><i>Lv.13</i></span>-->
 						<?php if($user_info['ipai']): ?><span class="yiyuan"></span><?php endif; ?>
 						<?php if($user_info['ishot']): ?><span class="icon6"></span><?php endif; ?>
 					</li>

                    <li data-url="<?php echo U('Home/Index/hot@', array('catid'=>$user_info['catid']));?>"><a href="<?php echo U('Home/Index/hot@', array('catid'=>$user_info['catid']));?>"><span class="mydata_key<?php if($user_info['type'] == 1): ?>2<?php endif; ?> bdr white px12 ellipsis"><?php echo ($user_info["category"]); ?></a></span></li>                           
...
$(window).resize(function() {
	...
	var fixleft = ($(window).width() - 1200) / 2  + 'px';
	var fixright = ($(window).width() - 1200)  + 'px';
	if( $(window).width() <1200){
		$(".left_body").css("right", fixright);
	}else{
		$(".left_body").css("right", fixleft);
	}
});
$(function() {
...
//	var flowmydata = $(".left_body");
	
		//浮动搜索框隐藏，淡入效果 
		
			if (top > 160) {
				$(".left_body").addClass("mydata_active");	
			}
			else {
				$(".left_body").removeClass("mydata_active");
		}
...
<?php else: ?><div class="right_body right">        
          <?php if($type=='all'){ ?>
    <div class="release_box cl50 weibo_content weibo_post_box">
                <div class="release_title">
                    <div class="idea left"></div>
