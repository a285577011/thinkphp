...
class BaseController extends AddonsController
{
    ...
    public function callback()
    {
        ...
        if ($is_login) {
            ...
            /* dpj 2016-03-17修改 */
            $uid =  $this->checkIsSync(array('type_uid' => $token['openid'], 'type' => $type)); // 返回uid
            /* if ($addon_config['bind'] && !$uid) {
            ...
            if($uid){
                // 如果存在uid，查询uid信息，判断是否完成注册
                $userinfo = D('Common/Member')->getInfoById($uid);
                // 如果没有，继续注册流程
                if(!$userinfo['username']){
                    redirect(U('ucenter/member/register@', array('type' => 'weixin')));
                }else{
                    //如果有，直接登录
                    $this->loginWithoutpwd($uid);exit;
                }
            }
            ...
            if(!$uid){
                ...
                /* 数据
                array (size=5)
                  'type' => string 'WEIXIN' (length=6)
                  'name' => string '别问我是谁' (length=15)
                  'nick' => string '别问我是谁' (length=15)
                  'head' => string 'http://wx.qlogo.cn/mmopen/icZynHSib9ic6dEI8I6Lw8hM7YiboaM8HYQvDwIcVzSEianCPpKAyskicphLVnQGjDUMOoC2EJgZSzfCmuREwqHlu0CDypUc2D68xy/0' (length=130)
                  'sex' => int 0 // 0男 1女
                 */
                $user_info['sex'] += 1;
                $uid = $this->addData($user_info);
                if(!$uid){
                    $this->error('获取信息失败，请重新扫描注册', addons_url('SyncLogin://Base/login',array('type'=>'weixin')));
                }else{
                    // 跳转注册
                    redirect(U('ucenter/member/register@', array('type' => 'weixin')));
                }
     ...
     * @author dpj 2016-03-17修改
     */
    public function checkIsSync($map = array())
    {
