...
var video = {
    css:'<style>.music_input{margin-bottom:20px;margin-top:20px;} .music_input dt{font-weight: bold; line-height:20px;}</style>',

    html: '<div class="video" style="position: absolute;background: #fff;z-index: 1001;border: 1px solid #ccc; padding: 0 20px;width: 500px;min-height:60px;height:auto;">' +
        '<a onclick="video.close($(this))" class="right" style="margin-top: 4px; font-size:16px;"><i class="icon icon-remove"></i></a><dl id="music_input" class="music_input"><dt>视频网址(或flash地址)：</dt><div style="font-size: 12px; line-height:30px;">（支持<a href="http://www.youku.com/" target="_blank">优酷</a>、' +
        ...
        '<input class="form-control pull-left" type="text" id="video_url" style="width:440px;height:28px;padding:0;margin-right:3px;"/>' +
        '<input type="button" class="btn btn-default" onclick="" value="搜索" data-role="search_video">' +
        '</dd></dl><div class="video_s_r"></div><input name="feed_type" value="wvideo" type="hidden">' +
        '<input name="title" class="extra" value="" type="hidden"><input name="video_url" class="extra" value="" type="hidden">' +
        '<input name="swf_src" class="extra" value="" type="hidden"><input name="img_url" class="extra" value="" type="hidden"><div class="clear"></div></div> <div class="clear"></div>',

    show_box: function () {
        $('#hook_show').html(this.css+this.html);
        bind_search_video();
    },
